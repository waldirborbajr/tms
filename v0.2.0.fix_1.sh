#!/bin/bash
# Script para aplicar melhorias ao tms - Versão corrigida

set -e

echo "🚀 Aplicando melhorias ao tms (versão corrigida)..."
...
# ========== 3. IMPORT_EXPORT.GO (SEM DUPLICATAS) ==========
...
# ========== 5. COMMANDS.GO (COM IMPORT DO STRINGS) ==========
...
# ========== 6. DISPLAY.GO (ADICIONAR VARIÁVEIS FALTANTES) ==========

if [ -f display.go ]; then
	# Verificar se as variáveis já existem
	if ! grep -q "Version" display.go; then
		# Adicionar variáveis no início do arquivo
		sed -i '1i\
var Version = "dev"\
